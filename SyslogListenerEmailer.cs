...
namespace OpenITTools_sysLog_Listener_Emailer
{
    class Program
    {
                ...
                for (int i = 1; i < TotalLines; i++)
                {
                    ...
                    while (pingable == false && NextLineIP == FerstLineIP)
                    {
                        ...
                        if (packet.Length != 0)
                        {
                            ...
                            string sysLogEP = groupEP.ToString();
                            string sysLogPK = packet + " - " + DateTime.Now;
                            string sysLogSubject = "Alert From " + sysLogEP + " " + sysLogPK;
                            string sysLogBody = "sysLog From " + sysLogEP + " " + sysLogPK + " - " + output2 + "\n ----FROM NODE " + FerstLineIP;
                            string sysLogshortMsg = output2;
                            string path = @"c:\sysLog.txt"; // Users\" + Environment.UserName + "\\sysLog.txt";
                            string bufferPath = @"c:\sysLogSendBuffer.txt";
                            ...
                            if (packet[0] == 0xff)
                            {
                                ...
                                try
                                {
                                    ...
                                    MailMessage mm = new MailMessage(FROMADDRESS, TOADDRESS, DateTime.Now + " - INVALID PACKET", DateTime.Now + " - sysLog INVALID PACKET");
                                    ...
                                    try
                                    {
                                        ...
                                        if (!File.Exists(path))
                                        {
                                            ...
                                            {
                                                sw.WriteLine("syslog.txt Log File Begin");
                                                sw.WriteLine("----------------------------------------------------------------------");
                                            }
                                        }

                                        // This text is always appended for a successful sysLog Email
                            ...
                            try
                            {
                                ...
                                MailMessage mm = new MailMessage(FROMADDRESS, TOADDRESS, sysLogSubject, sysLogshortMsg);
                                mm.BodyEncoding = UTF8Encoding.UTF8;
                                mm.DeliveryNotificationOptions = DeliveryNotificationOptions.OnFailure;
                                mm.DeliveryNotificationOptions = DeliveryNotificationOptions.OnSuccess;
                                mm.DeliveryNotificationOptions = DeliveryNotificationOptions.Delay;
                                ...
                                try
                                {
                                    ...
                                    using (StreamWriter sw = File.AppendText(path))
                                    {
                                        sw.WriteLine(DateTime.Now + " ----- " + sysLogBody);
                                        sw.WriteLine("----------------------------------------------------------------------");
                                    }
                                }
                                catch
                            ...
                            catch (Exception EXCEP)
                            {
                                ...
                                Console.WriteLine(DateTime.Now + " - Email Failed ----- " + EXCEP + " ----- " + sysLogBody);
                                ...
                                try
                                {
                                    ...
                                    using (StreamWriter sw = File.AppendText(path))
                                    {
                                        sw.WriteLine(DateTime.Now + " ----- Email Failure ----- " + EXCEP + " ----- " + sysLogBody);
                                        sw.WriteLine("----------------------------------------------------------------------");
                                    }
                                }
                                catch
